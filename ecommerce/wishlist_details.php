	...
	if(isset($_SESSION['user'])){
		...
		try{
			...
			foreach($stmt as $row){
				...
				$output .= "
					<tr>
						<td><button type='button' data-id='".$row['wishlistid']."' class='btn btn-danger btn-flat wishlist_delete'><i class='fa fa-remove'></i></button></td>
						<td><img src='".$image."' width='30px' height='30px'></td>
						<td>".$row['name']."</td>
						<td> ".number_format($row['price'], 2)."</td>
						<td class='input-group'>
							<span class='input-group-btn'>
            					<button type='button' id='minus' class='btn btn-default btn-flat minus' data-id='".$row['wishlistid']."'><i class='fa fa-minus'></i></button>
            				</span>
            				<input type='text' class='form-control' value='".$row['quantity']."' id='qty_".$row['wishlistid']."'>
				            <span class='input-group-btn'>
				                <button type='button' id='add' class='btn btn-default btn-flat add' data-id='".$row['wishlistid']."'><i class='fa fa-plus'></i>
				                </button>
				            </span>
						</td>
						<td> ".number_format($subtotal, 2)."</td>
						<td>
						<a href='sales1.php?pay=".$row['product_id']."'><button class='btn btn-warning btn-lg btn-flat' ><i class='fa fa-shopping-cart '></i></button></a>
						</td>
		...
		if(count($_SESSION['wishlist']) != 0){
			...
			foreach($_SESSION['wishlist'] as $row){
				...
				$product = $stmt->fetch();
				$image = (!empty($product['photo1'])) ? 'images/'.$product['photo1'] : 'images/noimage.jpg';
			...
			$output .= "
				<tr>
					<td colspan='6' align='center'>Shopping wishlist empty</td>
				<tr>
			";
		}
